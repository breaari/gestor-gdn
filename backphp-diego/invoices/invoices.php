...
function public_invoice(array $row): array {
  return [
    ...
    'payment_status' => $row['payment_status'] ?? null, // 'Pendiente' | 'Pagado'
...
$action = $_GET['action'] ?? null; // 'add_payment' etc.
...
try {
  switch ($method) {
    ...
    case 'GET': {
      ...
      $month       = int_or_null($_GET['month'] ?? null); // 1..12
      $from        = parse_datetime($_GET['date_from'] ?? null);
      $to          = parse_datetime($_GET['date_to']   ?? null);
      $q           = nonempty_str($_GET['q'] ?? null); // busca en archive_path
      $company_name= nonempty_str($_GET['company_name'] ?? null); // selector de razón social
      $supplier_q  = nonempty_str($_GET['supplier_q'] ?? null);   // buscador admin por proveedor

      $where  = [];
      $params = [];

      if ($supplier_id !== null) { $where[] = 'supplier_id = :sid'; $params[':sid'] = $supplier_id; }
      if ($category_id !== null) { $where[] = 'category_id = :cid'; $params[':cid'] = $category_id; }
      if ($locality_id !== null) { $where[] = 'locality_id = :lid'; $params[':lid'] = $locality_id; }
      if ($is_valid !== null)    { $where[] = 'is_valid = :iv';     $params[':iv']  = $is_valid; }
      if ($status !== null && in_array($status, $PAYMENT_STATUS, true)) {
        $where[] = 'payment_status = :ps'; $params[':ps'] = $status;
      }
      if ($company_name !== null) {
        $where[] = 'company_name = :cn'; $params[':cn'] = $company_name;
      }
      if ($month !== null && $month >= 1 && $month <= 12) {
        $where[] = 'upload_month = :um'; $params[':um'] = $month;
      }
      if ($from !== null) { $where[] = 'upload_date >= :df'; $params[':df'] = $from; }
      if ($to   !== null) { $where[] = 'upload_date <= :dt'; $params[':dt'] = $to; }
      if ($q    !== null) { $where[] = 'archive_path LIKE :q'; $params[':q'] = '%'.$q.'%'; }

      // Buscador admin por proveedor (razón/fantasía/email/CUIT)
      if ($supplier_q !== null) {
        ...
        $where[] = 'supplier_id IN (
                      ...
                      WHERE company_name LIKE :sq
                         ...
                         OR email        LIKE :sq' . ($digits ? ' OR cuit_or_cuil LIKE :sqd' : '') . '
                    )';
        $params[':sq'] = '%'.$supplier_q.'%';
        if ($digits) { $params[':sqd'] = '%'.$digits.'%'; }
      }

      $whereSql = $where ? ('WHERE '.implode(' AND ', $where)) : '';

      $total = (int)db_select("SELECT COUNT(*) FROM invoices $whereSql", $params)->fetchColumn();

      // 👇 NO usar placeholders para LIMIT/OFFSET
      $rows = db_select(
        "SELECT id,supplier_id,company_name,category_id,locality_id,payment_status,is_valid,invalid_reason,invalidated_at,upload_date,upload_month,archive_path
           FROM invoices
           $whereSql
      ...
      out(['invoices'=>$invoices, 'total'=>$total, 'limit'=>$limit, 'offset'=>$offset]);
    }

    /* ========== POST (create / add_payment) ========== */
    case 'POST': {
      // Sub-acción: agregar comprobante de transferencia
      if ($action === 'add_payment') {
        ...
        db_exec(
          'INSERT INTO invoices_payment (invoice_id, receipt_path, paid_at)
           VALUES (:inv, :rp, :pa)',
          [':inv'=>$invoice_id, ':rp'=>$receipt_path, ':pa'=>$paid_at]
        );

        if ($mark_as_paid === 1) {
          db_exec('UPDATE invoices SET payment_status = :ps WHERE id = :id', [':ps'=>'Pagado', ':id'=>$invoice_id]);
        }

        $pid = (int)last_id();
        $created = db_select(
          'SELECT id, invoice_id, receipt_path, paid_at, created_at, updated_at
             FROM invoices_payment WHERE id=:id',
          [':id'=>$pid]
        )->fetch();

        out(['message'=>'Comprobante registrado', 'payment'=>public_payment($created)], 201);
      }
      ...
      db_exec(
        ...
        [
          ':sid'=>$supplier_id, ':con'=>$company_name, ':cid'=>$category_id, ':lid'=>$locality_id,
          ':ps'=>$payment_status, ':iv'=>$is_valid, ':ir'=>$invalid_reason, ':ia'=>$invalidated_at,
          ':ud'=>$upload_date, ':ap'=>$archive_path
        ]
      );
    ...
    case 'PUT': {
      ...
      $curr = db_select('SELECT * FROM invoices WHERE id=:id LIMIT 1', [':id'=>$id])->fetch();
      ...
      $invalid_reason = array_key_exists('invalid_reason', $body) ? nonempty_str($body['invalid_reason']) : ($curr['invalid_reason'] ?? null);
      $invalidated_at = array_key_exists('invalidated_at', $body) ? parse_datetime($body['invalidated_at']) : ($curr['invalidated_at'] ?? null);

      if ($is_valid === 0 && $curr['is_valid'] == 1) {
        if ($invalid_reason === null) $invalid_reason = 'Marcada como no válida';
        if ($invalidated_at === null) $invalidated_at = date('Y-m-d H:i:s');
      }
      if ($is_valid === 1 && $curr['is_valid'] == 0) {
        $invalid_reason = null;
        $invalidated_at = null;
      }
      ...
      db_exec(
        "UPDATE invoices
            SET supplier_id=:sid, company_name=:con, category_id=:cid, locality_id=:lid,
                payment_status=:ps, is_valid=:iv, invalid_reason=:ir, invalidated_at=:ia,
                upload_date=:ud, archive_path=:ap
          WHERE id=:id",
        [
          ':sid'=>$supplier_id, ':con'=>$company_name, ':cid'=>$category_id, ':lid'=>$locality_id,
          ':ps'=>$payment_status, ':iv'=>$is_valid, ':ir'=>$invalid_reason, ':ia'=>$invalidated_at,
          ':ud'=>$upload_date, ':ap'=>$archive_path, ':id'=>$id]
      );
...
/* ========== DELETE: soft (default) u hard (hard=1) ========== */
case 'DELETE': {
  if (!$id) out(['message'=>'Falta id'], 400);

  $hard = parse_boolish($_GET['hard'] ?? null); // 1 => hard delete

  if ($hard === 1) {
    // HARD DELETE robusto: transacción + borra pagos SOLO si la tabla existe
    $exists = db_select('SELECT 1 FROM invoices WHERE id=:id LIMIT 1', [':id'=>$id])->fetchColumn();
    if (!$exists) out(['message'=>'No encontrado'], 404);

    try {
      db_exec('START TRANSACTION');

      // Si más adelante creás 'invoices_payment', esto se ejecutará; si no existe, se saltea.
      if (table_exists('invoices_payment')) {
        db_exec('DELETE FROM invoices_payment WHERE invoice_id = :id', [':id'=>$id]);
      }

      db_exec('DELETE FROM invoices WHERE id = :id', [':id'=>$id]);

      db_exec('COMMIT');
      out(['message'=>'Eliminada', 'id'=>(int)$id]);
    } catch (Throwable $e) {
      try { db_exec('ROLLBACK'); } catch (Throwable $ignored) {}
      out(['message'=>'No se pudo eliminar (hard).', 'detail'=>$e->getMessage()], 500);
    }
  }

  // SOFT DELETE (default): marcar como no válida
  $reason = nonempty_str($_GET['reason'] ?? null) ?? 'Marcada como no válida (DELETE)';
  try {
    db_exec(
      "UPDATE invoices
         SET is_valid=0, invalid_reason=:r, invalidated_at=NOW()
       WHERE id=:id",
      [':r'=>$reason, ':id'=>$id]
    );
    $row = db_select(
    ...
    out(['message'=>'Factura marcada como no válida', 'invoice'=>public_invoice($row)]);
  } catch (Throwable $e) {
    out(['message'=>'No se pudo invalidar (soft).', 'detail'=>$e->getMessage()], 500);
  }
} break;
