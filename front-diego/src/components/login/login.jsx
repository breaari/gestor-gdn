...
// import { login, clearAuthMessages } from "../../redux/users.slice";
...
//   // Estado global de auth (desde el slice)
...
//     if (!isValidCuit(cuitDigits)) {
//       setStep(1);
//       setLocalError("CUIT/CUIL inválido. Volvé al paso anterior.");
//       return;
//     }
...
//     try {
//       const user = await dispatch(login({ cuit_or_cuil: cuitDigits, password })).unwrap();
//       // (opcional) si usás este id en otros lados:
//       localStorage.setItem("authenticatedUserId", String(user.id));
//       navigate(`/manager/${user.id}/invoices`, { replace: true });
//     } catch (_) {
//       // El mensaje ya viene en authError desde el slice (rejected).
//     }
//   };
...
//         </div>
//         <p className="text-sm text-slate-500 mb-4">Accedé a tu cuenta para continuar</p>
...
//             </div>
//             <button
//               onClick={nextFromCuit}
//               className="w-full rounded-xl bg-azulceleste text-white py-2.5 font-medium hover:bg-azulceleste/90 active:scale-[.99]"
//             >
//               Siguiente
//             </button>
...
//               <div>
//                 <span className="font-medium text-azuloscuro">CUIT/CUIL:</span> {onlyDigits(cuit)}
//               </div>
//               <button type="button" className="text-azulceleste hover:underline" onClick={() => { setStep(1); dispatch(clearAuthMessages()); }}>
//                 Cambiar
//               </button>
//             </div>
//             <div>
...
//                   className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-azulceleste"
//                 />
//                 <button
//                   type="button"
//                   onClick={() => setShowPass((s) => !s)}
//                   className="absolute inset-y-0 right-0 px-3 text-slate-500 hover:text-azuloscuro"
  ...
  return (
    <div className="min-h-screen bg-azuloscuro flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Sin contraseña. Solo validamos tu CUIT/CUIL.</p>
