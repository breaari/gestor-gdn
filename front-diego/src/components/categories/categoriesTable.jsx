// // src/components/panel/CategoriesTable.jsx
...
// import Pagination from "../pagination"; // 👈 ruta correcta
...
//     setDelModal({ open: true, loading: false, error: "", category });
//   const closeDelete = () =>
//     setDelModal({ open: false, loading: false, error: "", category: null });

//   const confirmDelete = async () => {
//     if (!delModal.category?.id) return;
//     setDelModal((m) => ({ ...m, loading: true, error: "" }));
//     try {
//       await dispatch(deleteCategory(delModal.category.id)).unwrap();
//       closeDelete();
//       dispatch(fetchCategories());
//     } catch (e) {
//       setDelModal((m) => ({
...
//               <label className="field-label mb-2">Buscar categoría</label>
//               <input
//                 type="search"
...
//                       {delModal.category?.name || "esta categoría"}
//                     </span>
...
export default function CategoriesTable() {
  ...
  const activeFilters = useMemo(() => {
    const pills = [];
    if (q?.trim())
      pills.push({ k: "Buscar", v: q.trim(), clear: handleResetFilters });
    return pills;
  }, [q]);
  ...
  return (
    <div className="p-4 md:p-5">
      ...
      {/* ===== LISTA MOBILE (cards con acciones ARRIBA a la derecha) ===== */}
          ...
          list.map((c) => (
            <div key={c.id} className="rounded-xl border bg-white p-3 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                {/* Acciones: Editar + Eliminar (ghost) en la MISMA línea, arriba a la derecha */}
                <div className="flex items-center gap-2">
                  <EditCategory category={c} />
                  <button
                    type="button"
